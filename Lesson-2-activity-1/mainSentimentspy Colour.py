...
# Initialize colorama for colored output and auto-reset styles
colorama.init(autoreset=True)

print(f"{Fore.CYAN}🚀 Welcome to Sentiment Spy! {Style.RESET_ALL}")

user_name = input(f"{Fore.MAGENTA}Please enter your name: {Style.RESET_ALL} ").strip()
# Fallback if user doesn't provide a name
if not user_name:
    ...
    if user_input.lower() == "exit":
        ...
        if not conversation_history:
            ...
            for idx, (text, polarity, sentiment_type) in enumerate(conversation_history, start=1):
                # Choose color & emoji based on sentiment for history display
        ...
        continue # Return to the input prompt after displaying history

    # If not a command, analyze sentiment
    blob = TextBlob(user_input)
    polarity = blob.sentiment.polarity
    
    # Determine sentiment type based on polarity thresholds
    if polarity > 0.25:
